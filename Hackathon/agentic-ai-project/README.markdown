...
1. **Clone or Extract the Project**:
   - If you have a ZIP file, extract it:
     ```bash
     unzip agentic-ai-project.zip -d agentic-ai-project
     cd agentic-ai-project
     ```
   - Or clone if hosted in a repository:
     ```bash
...
3. **Install Dependencies**:
   ...
   ```
   - Ensures `google-generativeai==0.7.2` and `langchain-google-genai==0.0.16` to avoid `AttributeError: 'GenerationConfig' has no attribute 'Modality'`.

4. **Configure Environment**:
   - Copy the example `.env` file and update with your API key(s):
     ```bash
     cp .env.example .env
     ```
     Edit `.env`:
...
5. **Prepare Data**:
   - Place `AGENTIC AI.pdf` in `data/`.
   - If unavailable, create a dummy PDF or modify `create_faiss_index.py` to skip indexing.

6. **Create FAISS Index**:
   ```bash
   python create_faiss_index.py
   ```
   - Generates the vector store in `vectorstore/faiss_index/` for RAG.
